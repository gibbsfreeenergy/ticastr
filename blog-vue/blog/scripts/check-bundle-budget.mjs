...
const entryJsBudget = Number(process.env.PUBLIC_ENTRY_JS_BUDGET_KB || 640) * 1024;
const entryCssBudget = Number(process.env.PUBLIC_ENTRY_CSS_BUDGET_KB || 384) * 1024;
const chunkBudget = Number(process.env.PUBLIC_CHUNK_BUDGET_KB || 500) * 1024;
...
const chunks = fs.readdirSync(assetDirectory)
  .filter(file => file.endsWith(".js"))
  .map(file => ({ file, bytes: fs.statSync(path.join(assetDirectory, file)).size }))
...
for (const chunk of chunks) {
  if (chunk.bytes > chunkBudget) failures.push(`${chunk.file} ${(chunk.bytes / 1024).toFixed(1)} KB exceed ${chunkBudget / 1024} KB`);
}
